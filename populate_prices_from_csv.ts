...
// === CONFIGURATION ===
// WARNING: This service role key gives full DB access. Rotate it after use!
...
async function main() {
  const cards: { card_id: string, id: string }[] = [];
        ...
        if (cardData.tcgplayer?.prices) {
          for (const [variant, prices] of Object.entries(cardData.tcgplayer.prices)) {
            if (prices && typeof prices === 'object') {
              for (const [priceType, value] of Object.entries(prices as any)) {
