-- FINAL SOLUTION: Recreate tables without any unique constraints
...
SELECT * FROM card_wishlist;

-- Step 2: Drop existing tables
DROP TABLE IF EXISTS card_collections CASCADE;
DROP TABLE IF EXISTS card_wishlist CASCADE;

-- Step 3: Recreate card_collections table WITHOUT any unique constraints
...
-- Step 4: Recreate card_wishlist table WITHOUT any unique constraints
...
-- Step 5: Add indexes for performance (but NOT unique)
...
-- Step 8: Add update trigger for updated_at column
...
SELECT * FROM card_collections_backup;

INSERT INTO card_wishlist 
SELECT * FROM card_wishlist_backup;

-- Step 10: Verify no unique constraints exist
SELECT 
  tc.table_name, 
  tc.constraint_name, 
  tc.constraint_type,
...
-- Step 11: Test insert multiple copies
...
-- Step 12: Verify test inserts worked
SELECT 
...
-- Step 14: Clean up backup tables (optional)
